...
const FlightOffer: React.FC<FlightOfferProps> = ({offer}) => {
  ...
  const arrival = offer.itineraries[0].segments[offer.itineraries[0].segments.length - 1].arrival;
  ...
  return (
    <div className="border rounded-lg p-4 mb-4 shadow-md">
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-xl font-bold">{offer.price.total} {offer.price.currency}</span>
      </div>
      ...
      <div className="mt-2">
        <p className="text-sm">Asientos disponibles: {offer.numberOfBookableSeats}</p>
        <p className="text-sm">Última fecha para comprar: {offer.lastTicketingDate}</p>
      </div>
    </div>
